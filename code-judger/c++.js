...
const compile = (input_file_path, object_file_path) => {
  const pc = spawnSync("g++", [input_file_path, "-o", object_file_path]);
  return pc;
};
...
const run = (filename, options) => {
  ...
  const input_file_path = TMP_FILES + filename + ".cpp";
  ...
  run_status.output = c_execute.stdout ? c_execute.stdout.toString() : "";
